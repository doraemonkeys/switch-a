...
const CloseButton = ({ onClick, disabled }: { onClick: () => void; disabled?: boolean }) => (
    <button
        onClick={onClick}
        disabled={disabled}
...
export function GroupModal({ isOpen, onClose, onSubmit, initialData, title }: GroupModalProps) {
    ...
    useEffect(() => {
        if (initialData) {
            ...
            setFormData({ id: "", name: "", strategy: STRATEGIES.PRIORITY, priority: 0, weight: 0, enabled: true });
    ...
    const handleIdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setIdError(newId && !isValidId(newId) ? "ID can only contain lowercase letters, numbers, and hyphens" : null);
    };

    const inputClass = "w-full px-3 py-2 bg-bg-tertiary border border-border-light rounded-lg focus:outline-none focus:border-primary text-text-primary placeholder-text-muted";
            ...
            <div className="bg-bg-secondary w-full max-w-lg rounded-xl shadow-2xl border border-border-light max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor="enabled" className="text-sm font-medium text-text-secondary cursor-pointer">
                            Enable this group
                        </label>
                    </div>
