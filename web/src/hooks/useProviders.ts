...
export function useProviders(): UseProvidersResult {
    ...
    const createProvider = useCallback(async (data: ProviderInput): Promise<Provider> => {
        const provider = await api.providers.create(data)
        await refetch()
        return provider
    }, [api, refetch])

    const updateProvider = useCallback(async (id: string, data: ProviderInput): Promise<Provider> => {
    ...
    const resetProvider = useCallback(async (id: string): Promise<void> => {
