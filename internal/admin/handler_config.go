...
func (h *Handler) applyValidatedConfigUpdate(ctx context.Context, updates map[string]string) error {
	// Imports and direct updates share this lock so each response reflects one
	// complete mutation even when the storage implementation has its own transaction.
	h.configMutationMu.Lock()
	defer h.configMutationMu.Unlock()
	return h.store.SetConfigs(ctx, updates)
}
