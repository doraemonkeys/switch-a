...
func TestHTTPBodyObserverTracksReadCompletionAndFailure(t *testing.T) {
	t.Parallel()
...
func TestWriterToObserverPropagatesDestinationFailure(t *testing.T) {
	...
	if observer.observedBytes != int64(len(source.payload)) || !observer.SourceComplete() {
		t.Fatalf("source observation = bytes:%d complete:%t, want bytes:%d complete:true", observer.observedBytes, observer.SourceComplete(), len(source.payload))
	}
}
