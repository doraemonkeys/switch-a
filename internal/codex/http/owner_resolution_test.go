	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			for range 100 {
				...
				operation, err := runtime.Begin(context.Background(), request, codexAPIType, "route-preference", nil, nil)
				if err != nil {
					t.Fatal(err)
				}
...
func TestHTTPBeginPreservesContinuityRootCause(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(string(test.kind), func(t *testing.T) {
			...
			_, err := runtime.Begin(context.Background(), request, codexAPIType, "root-cause", nil, nil)
