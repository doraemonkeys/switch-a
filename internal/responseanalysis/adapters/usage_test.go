...
func TestExtractUsageUsesOnlyReviewedEnvelopeLocations(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "zero usage", data: "{\"usage\":{\"prompt_tokens\":0}}", want: true},
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if usage.PromptTokens.Value != test.prompt ||
				usage.CompletionTokens.Value != test.completion ||
				usage.TotalTokens.Value != test.total ||
				usage.CacheReadInputTokens.Value != test.cacheRead ||
				usage.ReasoningTokens.Value != test.reasoning {
				t.Fatalf("usage = %#v", usage)
			}
			if test.name == "root standard aliases and details" {
				if usage.ServiceTier != "flex" || usage.CacheCreation == nil ||
					usage.CacheCreation.InputTokens.Value != 3 ||
					usage.CacheCreation.Ephemeral1hInputTokens.Value != 2 ||
					usage.CacheCreation.Ephemeral5mInputTokens.Value != 1 {
...
func TestUsageIgnoresInvalidNumericFormsWithoutLosingValidFields(t *testing.T) {
	...
	if usage == nil || usage.PromptTokens.Value != 7 || usage.CompletionTokens.Value != 3 ||
		usage.TotalTokens.Present || usage.CacheReadInputTokens.Present ||
		usage.ReasoningTokens.Value != 2 {
		t.Fatalf("usage = %#v", usage)
	}
}
