...
func TestV5BConfigRevisionRestartAndProviderCascade(t *testing.T) {
	...
	providerA := persistenceProvider(persistenceProviderA, &groupID)
	providerB := persistenceProvider(persistenceProviderB, &groupID)
...
func persistenceProvider(id string, groupID *string) model.Provider {
	return model.Provider{
		ID: id, Name: id, APIKey: "secret", AuthMode: "bearer", Enabled: true,
		GroupID: groupID, Weight: 1, Vendor: "persistence-vendor",
		FailoverScope: model.ScopeAny, AcceptFailover: model.ScopeAny,
		APITypes: []model.ProviderAPIType{{
			ProviderID: id, APIType: proxy.APITypeCodex, BaseURL: "https://example.invalid", APIKey: "secret",
		}},
	}
}
