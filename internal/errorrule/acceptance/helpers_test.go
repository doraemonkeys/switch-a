...
func newProxyHarness(t *testing.T, options proxyHarnessOptions) *proxyHarness {
	...
	primaryProvider, primaryCredential := newAcceptanceProvider(
		t,
		primaryProviderID,
		"V5B Primary",
		options.primary.URL(),
	...
	providers := []model.Provider{primaryProvider}
	credentials := []credentialsession.Session{primaryCredential}
	if options.secondary != nil {
		secondaryProvider, secondaryCredential := newAcceptanceProvider(
			t,
			secondaryProviderID,
			"V5B Secondary",
		...
		providers = append(providers, secondaryProvider)
		credentials = append(credentials, secondaryCredential)
	}
	...
	if err := backend.ApplyConfigImport(ctx, &store.ConfigImportBundle{
		...
		CredentialSessions: credentials,
		Providers:          providers,
...
) (model.Provider, credentialsession.Session) {
	t.Helper()
	const vendor = "v5b-vendor"
	session, route := newStaticCredentialRoute(t, id, proxy.APITypeCodex, vendor, "v5b-secret")
	provider := model.Provider{
		ID: id, Name: name, AuthMode: "bearer", Enabled: true,
		GroupID: groupID, Priority: priority, Weight: 1, MaxRetries: maxRetries,
		Vendor: vendor, FailoverScope: model.ScopeAny, AcceptFailover: model.ScopeAny,
		APITypes: []model.ProviderAPIType{{
			ProviderID: id, APIType: proxy.APITypeCodex, BaseURL: baseURL,
		}},
...
func newStaticCredentialRoute(
	t *testing.T,
	routeTargetID, apiType, vendor, secret string,
	...
	if err != nil {
		t.Fatalf("snapshot credential session %q: %v", session.ID, err)
	}
