...
const SwitchReasonUsageLimitReached = "usage_limit_reached"
...
func shouldForceProviderSwitch(statusCode int) bool {
	return statusCode == http.StatusPaymentRequired || statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden
}

func shouldTrackWebSocketFailureInHealth(result *WebSocketResult) bool {
	if result == nil || result.HandshakeStatusCode == http.StatusUnauthorized {
		return false
	}
	...
	if result.UpstreamError.StatusCode > 0 &&
		(result.UpstreamError.StatusCode == http.StatusUnauthorized ||
			(result.UpstreamError.StatusCode == http.StatusForbidden && errorType == "auth_error")) {
		return false
	}
