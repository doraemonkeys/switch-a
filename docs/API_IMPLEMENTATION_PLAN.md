...
### 阶段 6：配置导出/导入 API
- **导出端点**: `GET /admin/api/config/export`
- **导入端点**: `POST /admin/api/config/import?dry_run=true|false`
- **导出格式版本**: `1.0`
- **功能**: 
  - 导出：获取所有 providers、groups、settings 的完整配置
  - 导入预览：dry_run=true 时返回变更预览和警告
  - 实际导入：dry_run=false 时执行导入（覆盖策略）
- **冲突处理**: ID 相同时覆盖现有配置
