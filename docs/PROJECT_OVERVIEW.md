...
| Testing  | Go test (CI: ≥90% total, ≥70% package), Vitest (≥20% coverage gate) |
...
- **Sticky** (`internal/selector/`): Soft Provider preference within routing, health, and concurrency constraints. `sticky_mode`: `off` / `api_type` (default) / `model`; `sticky_ttl`: 604800 seconds (7 days) by default. Codex keys include the persistent client's canonical scope; explicit replacement-Key binding retains that scope.
...
- **Provider Cookie lifecycle** (`internal/codex/cookie/`): Only a client-returned gateway handle authorizes Jar reuse. Missing or invalid handles use request-local overlays; only the selected attempt's live Cookies create a binding and encrypted entries in one transaction. HTTP emits the handle after commit; WebSocket reserves an in-memory token before `101` and persists it at the final Cookie boundary. Transient empty requests and abandoned attempts create no binding. Live request leases protect Jars through response/session teardown; cleanup removes idle empty Jars, and global capacity reclaims empty, never-returned, then least recently reused inactive Jars. Exhaustion reports `CODEX_COOKIE_CAPACITY_EXHAUSTED`; existing-state read failures remain explicit.
...
- **Codex conversation continuation** (`internal/codex/continuation/`): Each Provider has independent outbound and inbound boundaries: `none`, `same_identity`, or `any`. New and existing Providers default to outbound `any` and inbound `none`. Codex HTTP/WS prefer the conversation’s current Provider; moving requires both boundaries to allow it. The serving relationship persists across restarts separately from original state ownership; wire state remains unchanged. Vendor Isolation retains its existing true-failover semantics. Pre-visible attempts may be replaced; visible WebSocket failures recover through client reconnect. The former global recovery setting is retired.
- **Provider / CredentialSession / Authority** (`internal/codex/credentialsession/`, `internal/codex/identity/`): Provider defines a route target, CredentialSession owns credentials and authentication lifecycle, and Authority identifies the upstream ownership boundary. Switching Providers need not change accounts. Provider forms edit API keys directly as route-local replacements on save. GPT reconnection takes effect immediately for the shared session, using its current version under the mutation lease so background updates do not invalidate sign-in.
...
- **Downstream client identity** (`internal/codex/clientidentity/`): Resolves entry API Keys to a persistent client and legacy scope aliases shared by reference learning, continuity and sticky routing. Explicit Key binding preserves identity; conflicting established ownership is rejected. Reference selection shows clients by their latest original HTTP request, WebSocket handshake or logical turn, with observed type/platform/version and exact local time. Activity is deployment-local, independent of reference learning and portable samples; shared Keys share one client identity.
- **Login device & client profile** (`internal/codex/clientdisguise/`): Each resolved credential session creates an independent device/generation in the same transaction, including inline Provider keys and imported credentials; shared Providers reuse it. Profiles can be configured before the first request. Same-account refresh/reauthentication preserves the device, account changes archive it. Selectable environments distinguish Desktop, interactive CLI/TUI (`codex-tui`), non-interactive CLI/exec (`codex_exec`), and the generic default identity (`codex_cli_rs`). Browser Use (`codex-browser-use`) is an automatic auxiliary request role: its samples are collected separately and cannot become the primary login binding. A Browser Use first request uses the device without binding; an ordinary request or explicit selection establishes the primary environment. Tuple-specific immutable revisions follow a designated reference or a pinned revision. Original ingress evidence independently selects the request role: ordinary requests use the selected primary client; Browser Use requests use that environment's Browser Use UA, sharing its Codex release and host while retaining their own caller build and terminal. Known versions and host tuples apply even without a complete UA sample; unknown OS releases are not carried across platforms. One frozen request profile drives HTTP/WS headers and protocol metadata. Complete samples retain observed thread-originator differences; new releases never inherit an older release's UA. Other unspecified observations remain unchanged. Transport samples apply only supported, explicitly recorded settings.
- **Codex OAuth login** (`internal/providerauth/`): **Settings → 认证与账号 → Codex OAuth 登录** selects the authorization URL's `originator`, defaulting to `codex_cli_rs`. `codex_vscode` and custom values are supported; blank uses the default. Changes apply to the next login/reauthentication and travel with settings exports.
- **OAuth refresh recovery** (`internal/providerauth/`): Successful token exchanges remain in memory until their credential commit succeeds or a persisted replacement supersedes them. Later requests retry the local commit before another exchange; metadata updates preserve the pending result. Recovery is scoped to the credential session and does not survive process termination before persistence.
- **Account request identity** (`internal/providerauth/accountclient/`): Token refresh, usage queries and reauthentication use the credential session’s selected profile UA, including its version source. Provider routing policy does not govern shared account maintenance. Each operation freezes the selection; token refresh also sends the originator paired with a selected profile UA, while authorization-code exchange and usage queries keep their UA-only projection. The gateway-UA fallback adds no originator; account endpoints receive no conversation headers. Without a sampled UA, **Settings → GPT Account Requests** chooses the fallback: the built-in Codex Desktop UA with the shared Codex CLI official stable version by default, or `switch-a/<build version>`. Bound profile UAs and their version rules always take priority; each operation logs the chosen source.
...
- **Codex CLI official stable version** (`internal/codex/clientdisguise/officialversion/`): In Client disguise → Login profiles, choose **Version source → 同步 Codex CLI 官方稳定版** to follow `openai/codex` CLI releases independently of the built-in/reference environment. Checks run every 6 hours while selected for a login or the global account fallback; **Check now** refreshes immediately. Failed checks retry after 15 minutes and retain the last successful version; before the first success, the profile version applies. UA product/version fields change together; OS, Desktop/Browser Use caller builds, conversation IDs and cache keys stay unchanged. Each request/WS connection freezes its version. Backups retain the selection; the release cache is local to the deployment.
- **Portable Codex restore** (`internal/store/*codex_transfer*`, `internal/codex/keyring/`): Config `codex_state` merges devices/history, profiles/reference tracks, client aliases, continuity, HMAC material and sticky bindings atomically. Selected imports follow referenced state; settings-only preserves it. ChatGPT restores as pending reauthentication; same-account verification reuses its device. Conflicts roll back and caches publish only committed state.
- **Disclosure / ClientVisible** (`internal/upstreamtransport/`, `internal/codex/websocket/boundaries.go`): Possible upstream request disclosure and client-visible output are separate lifecycle boundaries governing recovery. HTTP exchanges and WebSocket handshakes share transport disclosure observation. Proven pre-write failures and explicit HTTP rejection of a WebSocket upgrade release only that attempt's new pending claims; prior owners and uncertain handshakes retain their constraints. An ordinary WebSocket `101` does not establish business visibility.
- **Provider creation defaults** (`internal/defaults/`): Concurrency 100, 4 retries, 1-second initial delay, no maximum delay, 3× multiplier, and jitter enabled. Explicit zero concurrency means unlimited; explicit zero retries/backoff disables them.
...
- **WebSocket message size**: Global configuration → 超时与重试 controls `websocket_max_message_size_mib` (default 128 MiB) for each complete message in both directions. Each new connection freezes the setting for model probing, forwarding, and provider replacements; existing connections retain their limit.
- **Requested reasoning**: HTTP logs describe the request body; WebSocket live/session logs describe the most recent client `response.create`. Each new turn replaces the observation (including omitted settings); provider replay does not advance it. Structured `websocket.request_reasoning_observed` events preserve per-turn diagnostics.
- **Usage-limit policy & health** (`internal/model/provider_usage_limit_policy.go`, `internal/errorrule/health.go`): Provider-scoped `usage_limit_policy` defaults to `suspend` for GPT-login-only targets and `switch_provider` otherwise; explicit policies take precedence. Routing-away decisions and health verdicts are separate.
...
For enabled Codex profiles, original input remains the source for routing, ownership, error analysis and replay; physical HTTP/WS output replaces installation IDs with the frozen target's persistent device ID and applies observed features. Conversation/session, turn, request, window and parent-reference IDs and cache keys pass through unchanged; cross-account continuation remains owned by the continuity policy. Disguise failures carry a diagnostic ID and original/derived field evidence.
...
2. **Optional client model probing** (disabled by default; independent duration, decoded-byte and work budgets; queue frames for first delivery) → `internal/websocketproxy/`, `internal/codex/websocketprotocol/`
...
| Token Usage | `/token-usage` (`/admin/token-usage`) | Token analytics by model/provider, filterable by original client API Key (including unregistered keys); older or unattributable logs remain unassigned. Observed-data quality is unassessed when no usage is observed (`quality_rate: null`) |
