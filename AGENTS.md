...
| --------------------- | -------------------------------- | -------------------------------------------------- |
| `X-Client-Request-Id` | 请求关联标识；已核对的本地 Codex Responses HTTP/WS 路径复用 `Thread-Id` | 按具体协议解释，不能仅凭字段名推断粒度 |
| `X-Codex-Window-Id`   | 观察到的上下文窗口标识，当前格式为 `Thread-Id:index` | 可关联当前线程及其上下文窗口代际，不代表账号 |
...
- 客户端伪装仅替换安装／设备 ID 及明确选定的客户端特征；会话、线程、回合、请求、窗口、父子引用和缓存键保持原值。跨账号继续对话由连续性机制决定。
- switch-a 应该尽量保持客户端的原有行为特征，不得擅自修改添加不合理的参数，例如自动添加客户端未处理的 `Cookie`、改变 `Accept-Encoding` 的值等行为特征。
