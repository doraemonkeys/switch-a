...
func (mw *msgWriter) close() {
	if mw.c.client {
		_ = mw.c.writeFrameMu.Lock(context.Background(), nil, false)
...
const controlFrameIOTimeout = 5 * time.Second
...
func (c *Conn) writeFrame(ctx context.Context, fin bool, flate bool, opcode opcode, p []byte) (_ int, err error) {
	control := opcode == opPing || opcode == opPong || opcode == opClose
	err = c.writeFrameMu.Lock(ctx, c.closed, control)
	if err != nil {
		return 0, err
	}
	defer c.writeFrameMu.Unlock()

	// A control frame cannot interrupt the frame already on the wire. Give its
	// physical write a separate budget once the next boundary becomes available;
	// the caller's deadline still bounds the complete operation.
	if control {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, controlFrameIOTimeout)
		defer cancel()
	}
